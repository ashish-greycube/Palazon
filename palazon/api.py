...
def set_items(self,change_qty=0):
	print(self)
	self = frappe.get_doc("Sales Order", self)
		...
		if i.item_code:
			...
			if bom_no:
				...
				for item in sorted(item_dict.values(), key=lambda d: d['idx']):
					if item.idx is None:
						# items_parent = frappe.db.sql("""select item_name from `tabBOM Item` where bom_no=(select name from `tabBOM` where item_name = %s and is_default=1 and docstatus != 2)""", item.item_name,as_dict=True)
						# items_parent = frappe.db.sql("""select item_name from `tabBOM Item` where bom_no=(select parent from `tabBOM Item` where item_name = %s and docstatus != 2)""", item.item_name,as_dict=True)
						#items_parent =frappe.db.sql("""select item_name from `tabBOM Item` where item_name = %s and docstatus != 2 and bom_no=(select name from `tabBOM` where item_name = %s and is_default=1 )""", (item.item_name,item.item_name),as_dict=True)
						print(bom_no)
						print("---------")
